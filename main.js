    ...
    async function fetchUserData(uuid) {
        ...
        try {
            const response = await axios.get(`https://backend-develop-8a00.up.railway.app/api/annexes/get-fatca-info-by-uuid/${uuid}/`);
    ...
    const forms = [
        ...
        //    questions: [
        //      
        //        { text: "Ingrese su Número de Identificación del Empleador (EIN).", type: "input" }
        //    ]
        //},
        //{
        //    form_no: 1,
        ...
        //    questions: [
        //      
        //        { text: "Por favor, revise los términos y condiciones.", type: "terms" },
        //        { text: "Por favor, revise el documento PDF.", type: "img" }
        //    ]
        //}
    ];
    ...
    function filtrarFormularios() {
        ...
        if (!userMock || !Array.isArray(userMock.form_req)) {
            console.error("userMock o userMock.form_req no están definidos o no es un arreglo.");
            return []; // Retornar un arreglo vacío si no están definidos
        }
        const formulariosFiltrados = forms.filter(form => userMock.form_req.includes(form.form_no));
    ...
    function loadQuestion() {
        ...
        if (question.type === "img") {
            ...
            elements.questionSection.innerHTML = `
                <h2 class="text-[#00457C] font-medium font-semibold">Pregunta ${currentQuestion + 1}/${forms[currentForm].questions.length}</h2>
                ${questionHTML}
            `;
            // Rellenar la respuesta si ya existe
            if (answers[currentForm] && answers[currentForm][currentQuestion] !== undefined) {
                const respuesta = answers[currentForm][currentQuestion];
                if (question.type === "radio") {
                    const radioButtons = document.getElementsByName(`question-${currentForm}-${currentQuestion}`);
    ...
    function generateQuestionHTML(question) {
        if (question.type === "radio") {
                ...
                <div class="flex gap-4 mt-4">
                    ${question.options.map(option => `
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="question-${currentForm}-${currentQuestion}" value="${option}" class="hidden peer" ${answers[currentForm]?.[currentQuestion] === option ? "checked" : ""}>
            ...
            return `
                <div class="bg-blue-50 p-4 rounded-lg">
                    <p class="class="text-[#00457C] text-md font-bold ">${question.text}</p>
                    <input type="text" class="mt-2 p-2 border border-gray-300 rounded-lg w-full" value="${answers[currentForm]?.[currentQuestion] || ""}" />
                </div>
            `;
            ...
            return `
                <div class="bg-blue-50 p-4 rounded-lg">
                    <p class="text-[#00457C] text-lg font-regular">${question.text}</p>
                    <input type="date" class="mt-2 p-2 border border-gray-300 rounded-lg w-full" value="${answers[currentForm]?.[currentQuestion] || ""}" />
                </div>
            `;
        } 
    }
    ...
    function toggleButtonVisibility() {
        ...
        const esUltimaPregunta = currentQuestion === forms[currentForm].questions.length - 1;
        if (elements.btnSiguiente) {
            elements.btnSiguiente.style.display = "flex";
            elements.btnSiguiente.textContent = (esUltimoFormulario && esUltimaPregunta) ? "Finalizar" : "Siguiente";
        }
    ...
    function hideSignatureSection() {
        ...
        elements.btnSiguiente.style.display = "flex";
        elements.btnAnterior.style.display = (currentForm > 0 || currentQuestion > 0) ? "flex" : "none";
        btnAnteriorSignature.style.display = "none"; // Ocultar botón "Anterior" de firma
    }
    ...
    function handleSubmitForms() {
        ...
        axios.put('https://backend-develop-8a00.up.railway.app/api/annexes/update-fatca-info/', data)
            .then(response => {
                Swal.fire({
                    icon: 'success',
                    title: '¡Éxito!',
